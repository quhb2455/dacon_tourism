...
def plot_images(images: Tensor, title: str, images_per_row: int):
    ...
    img = torchvision.utils.make_grid(images, nrow=images_per_row).permute(1, 2, 0).numpy() * 255
    cv2.imwrite(title + '.jpg', img)
    # save_image(torchvision.utils.make_grid(images, nrow=images_per_row).permute(1, 2, 0),'demo.jpg')
